...
public sealed class UpdateTenantEntitlementsHandler(
    ITenantReadRepository tenantReadRepository,
    ITenantWriteRepository tenantWriteRepository,
    ICacheService cacheService,
    IUnitOfWork unitOfWork)
    : IRequestHandler<UpdateTenantEntitlementsCommand, Result<TenantResponse>>
{
    public async Task<Result<TenantResponse>> Handle(
        UpdateTenantEntitlementsCommand request,
        CancellationToken cancellationToken)
    {
        ...
        tenant.UpdateEntitlements(request.Entitlements);
